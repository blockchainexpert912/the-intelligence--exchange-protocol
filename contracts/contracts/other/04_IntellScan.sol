...
contract IntellScan is Ownable {
    ...
    function intellModelNFT() public view returns (IIntellModelNFTInDetail) {
        return
    ...
    function intellModelNFTCollectionDetail(
        address _owner
    ) external view returns (ModelMetadata[] memory) {
        uint256 tokenCount = intellModelNFT().balanceOf(_owner);
        uint256[] memory tokenIds = intellModelNFT().walletOfOwner(_owner);

        ModelMetadata[] memory intellModelNFTCollections = new ModelMetadata[](
            tokenCount
        );
        ...
        for (uint256 i = 0; i < tokenCount; i++) {
            if (factory().getShareNFTAddr(tokenIds[i]) != address(0)) {
                ...
                intellModelNFTCollections[i] = ModelMetadata(
                    tokenIds[i], // token id
                    intellModelNFT().modelIdByTokenId(tokenIds[i]), // model id
        ...
        return intellModelNFTCollections;
    }

    function singleIntellModelNFTDetail(
        uint256 _tokenId
    ) external view returns (ModelMetadata memory) {
        ...
        if (factory().getShareNFTAddr(_tokenId) != address(0)) {
            ...
            return
                ModelMetadata(
                    _tokenId, // token id
                    intellModelNFT().modelIdByTokenId(_tokenId), // model id
                    _shareMetadataNFT.name(), // name
                    _shareMetadataNFT.symbol(), // symbol
                    _shareNFT.totalSupply(), // so far total supply minted
                    _shareNFT.MAX_TOTAL_SUPPLY(), // max total supply
    ...
    function shareNFTCollectionDetail(
        ...
        for (uint256 i = 0; i < tokenCount; i++) {
            shareNFTCollections[i] = ShareMetadata(
                ...
                tokenId, // token id
                intellModelNFT().modelIdByTokenId(tokenId), // model id
                _shareMetadataNFT.name(), // name
                _shareMetadataNFT.symbol(), // symbol
                _shareNFT.totalSupply(), // so far total supply minted
                _shareNFT.MAX_TOTAL_SUPPLY(), // max total supply
